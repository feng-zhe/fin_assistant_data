...
yahoo.recreate = function(symbols, callback) {
    ...
    MongoClient.connect(dbUri, (err, db) => {
        ...
        db.collection(collectionName).drop(function(err, res) {
            if (err) {
                console.log('drop collection failed'); // if drop failed, we continue because it is the first time
            } else {
                console.log('collection dropped, response:' + res);
            }
            // create index
                ...
                (function inner_func(symbolIndex) {
                    ...
                    get_stock_csv(options, function(docs) {
                        db.collection(collectionName)
                            ...
                            .catch((err) => {
                                console.log('insert block failed, error: ' + err);
                            });
                    });
                })(0);
            });
            ...
            cursor.next(function(err, last) {
                ...
                get_stock_csv(options, function(docs) {
                    ...
                    if (docs.length === 0) { // skip this stock
                        console.log('no update data found for stock: ' + symbols[symbolIndex]);
                        inner_func(symbolIndex + 1);
                        return;
                    } else {
                        db.collection(collectionName)
